...
$Config = @{
    Description = 'GoogleDriveFS'
    DriveRoot ='C:'
    FolderInstall = 'Tools'
    FileInstall = 'GoogleDriveFSSetup.exe'
    FileArguments = ("/S")
    RootKey = 'HKLM:\Software\ManutTools\GoogleDriveFS'
    KeyName = '67.0.2'
    UrlDownloadInstall = 'https://dl.google.com/drive-file-stream/GoogleDriveFSSetup.exe'
    UrlDownloadRemoval = ''
}
...
If( $command -eq $false ) { Write-Host "Comando => .\GoogleDriveFSTools.ps1 [ install | download ]  " }
