...
<#
Scripts para instalação modo silencioso
FortiClient.msi /quiet /norestart INSTALLLEVEL=3 # Não funcionando
FortiClientVPN.exe /quiet /norestart # Não funcionando
...
$Config = @{
    Description = 'FortiClientSSLVPN'
    DriveRoot ='C:'
    FolderInstall = 'Tools'
    FileInstall = 'FortiClientSSLVPN-7.5.0.exe'
    FileArguments = ("/quiet", "/norestart", "/INSTALLLEVEL=3")
    RootKey = 'HKLM:\Software\ManutTools\FortiClientSSLVPN'
    KeyName = '7.5.0'
    UrlDownloadInstall = 'https://filestore.fortinet.com/forticlient/FortiClientVPNOnlineInstaller.exe'
    UrlDownloadRemoval = ''
}
...
If( $command -eq $false ) { Write-Host "Comando => .\FortiClientSSLVPNTools.ps1 [ install | download ] " }
