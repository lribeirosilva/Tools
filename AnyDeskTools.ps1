...
$Config = @{
    ...
    DriveRoot ='C:'
    FolderInstall = 'Tools'
    FileInstall = 'AnyDesk.exe'
    FileArguments = ("--install", "--start-with-win", "--create-shortcuts", "--create-desktop-icon", "--silent")
    RootKey = 'HKLM:\Software\ManutTools\AnyDesk'
    KeyName = '7.1.6'
    UrlDownloadInstall = 'https://download.anydesk.com/AnyDesk.exe'
    UrlDownloadRemoval = ''
}
...
If( $command -eq $false ) { Write-Host "Comando => .\AnyDeskTools.ps1 [ install | download ] " }
